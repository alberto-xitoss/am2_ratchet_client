...
class Server implements MessageComponentInterface
{
    ...
    public function onMessage(ConnectionInterface $client, $message)
    {
        ...
        if (!empty($datas->message)) {
            $this->CI->load->model('chat_messages_model');
            $save = $this->CI->chat_messages_model->preSave($datas);
        }
        ...
        if (!empty($datas->user_id) && $datas->user_id !== $client->resourceId) {
            ...
            if (!empty($this->CI->ratchet_client->callback['auth']) && empty($client->subscriber_id)) {
                ...
                // Output
                if ($this->CI->ratchet_client->debug) {
                    $this->user_cliente[$client->resourceId] = $datas->user_id;
    ...
    public function onClose(ConnectionInterface $connection)
    {
        ...
        $message = ['user_id' => $this->user_cliente[$connection->resourceId],'message' => '', 'status' => 'disconnected'];

        foreach ($this->clients as $user) {
            $this->send_message($user, json_encode($message), $client);
        }
